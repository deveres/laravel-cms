...
class LString
{
     ...
     * @return string
     */
    static function convertCase($string, $way = false)
    {
     ...
     */
    static function getRandomString($length, $dictionary = false)
    {
        $string = '';
        if ($dictionary === false) {
            if ($length <= 20) {
     ...
     */
    static function randomizeCapsLock($string)
    {
     ...
     */
    static function convertTextToUrl($text)
    {
     ...
     */
    static function getTranslite($text)
    {
        $rules = array('а' => 'a', 'б' => 'b', 'в' => 'v', 'г' => 'g', 'д' => 'd', 'е' => 'e', 'ё' => 'jo', 'ж' => 'zh', 'з' => 'z', 'и' => 'i', 'й' => 'j', 'к' => 'k', 'л' => 'l', 'м' => 'm', 'н' => 'n', 'о' => 'o', 'п' => 'p', 'р' => 'r', 'с' => 's', 'т' => 't', 'у' => 'u', 'ф' => 'f', 'х' => 'h', 'ц' => 'c', 'ч' => 'ch', 'ш' => 'sh', 'щ' => 'w', 'ъ' => '#', 'ы' => 'y', 'ь' => '', 'э' => 'je', 'ю' => 'ju', 'я' => 'ja');
     ...
     */
    static function getNumberWord($value, $names)
    {
        $temp = strval($value);
    ...
    public static function camelStyle($string, $up_first = false)
    {
        ...
        if ($up_first) $string = ucfirst($string);
     ...
     * @version  1.0.0
     */
    static function utf8_strpos($haystack, $needle, $offset = null)
    {
     ...
     * @subpackage strings
     */
    static function utf8_strrpos($haystack, $needle, $offset = null)
    {
        if ($offset === null or $offset < 0) $offset = 0;
        if (function_exists('mb_strpos')) return mb_strrpos($haystack, $needle, $offset, 'utf-8');
        if (function_exists('iconv_strrpos')) return iconv_strrpos($haystack, $needle, $offset, 'utf-8');
        return false;

        $byte_pos = $offset;
        do if (($byte_pos = strrpos($haystack, $needle, $byte_pos)) === false) return false;
        while (($char_pos = self::utf8_strlen(substr($haystack, 0, $byte_pos++))) < $offset);
        return $char_pos;
    }
    ...
    */
     ...
     */
    static function unparseString($string, $delim1, $delim2 = false)
    {
        return self::unparse($string, $delim1, $delim2);
    }

    static function unparse($string, $delim1, $delim2 = false)
    {
    ...
    static function compareVersions($what, $with)
    {
    ...
    static function lentext($str, $dl)
    {
        $konec = "";
    ...
    static function urlTranslite($text)
    {
    ...
    static function truncate($string, $length = 327, $etc = '...')
    {
        if ($length == 0) {
            return '';
        }
