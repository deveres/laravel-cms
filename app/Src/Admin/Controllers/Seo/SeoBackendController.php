...
class SeoBackendController extends BackendController
{
    ...
    protected function form($id = 0)
    {
        ...
        $form->tab('Общее', function (Form $form) use ($id) {
            ...
            $form->translit('alias', 'log_name', 'lock_alias', $item->lock_alias,
                'Алиас')->setLabelClass(['required'])->rules(
                function ($form) {
        ...
        foreach ($langs as $one) {
            $form->tab($one['name'], function (Form $form) use ($one, $id) {
                ...
                $form->textarea('translate[' . $one['alias'] . '][introtext]',
                    'Короткое описание')->rows(4)->default($translation ? $translation->introtext : '');
                $form->ckeditor('translate[' . $one['alias'] . '][text]',
                    'Описание')->default($translation ? $translation->text : '');
                $form->divider();
                $form->text('translate[' . $one['alias'] . '][seo_h1]',
                    'SEO H1')->default($translation ? $translation->seo_h1 : '');
                $form->text('translate[' . $one['alias'] . '][seo_title]',
                    'Meta Title')->default($translation ? $translation->seo_title : '');
                $form->textarea('translate[' . $one['alias'] . '][seo_keywords]',
                    'Meta Keywords')->rows(4)->default($translation ? $translation->seo_keywords : '');
                $form->textarea('translate[' . $one['alias'] . '][seo_description]',
                    'Meta Description')->rows(4)->default($translation ? $translation->seo_description : '');
